...
def getFormattedSeasonName():
    return re.sub('_',' ',getSeasonInfoDB()['seasonName'])
...
def startVoting():
    ...
    botMessage += f'\nVote to <@{str(botID)}> with `sp/vote [keyword] [keyletters]`.'
    botMessage += f' View more voting commands in <#{str(channelIDs['commandListing'])}>.'
    deadline = None
